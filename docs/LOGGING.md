...
**structured logging is not** (task 26). Today the app logs readable single lines to stdout -
...
structured and nothing is shipped anywhere.

Collecting logs off the instance is set up outside this repo. What this repo
owes it is below: one queryable line per event - still to do - and a health
endpoint worth polling, which now exists.
